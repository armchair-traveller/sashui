import React, { Fragment, createContext, createRef, useCallback, useContext, useMemo, useReducer, useRef } from 'react'
...
let reducers = {
  [ActionTypes.CloseListbox](state) {
    ...
    return Object.assign(Object.assign({}, state), { activeOptionIndex: null, listboxState: ListboxStates.Closed })
  },
  [ActionTypes.OpenListbox](state) {
    if (state.disabled) return state
    if (state.listboxState === ListboxStates.Open) return state
    return Object.assign(Object.assign({}, state), { listboxState: ListboxStates.Open })
  },
  [ActionTypes.SetDisabled](state, action) {
    if (state.disabled === action.disabled) return state
    return Object.assign(Object.assign({}, state), { disabled: action.disabled })
  },
  [ActionTypes.SetOrientation](state, action) {
    if (state.orientation === action.orientation) return state
    return Object.assign(Object.assign({}, state), { orientation: action.orientation })
  },
  [ActionTypes.GoToOption](state, action) {
    ...
    return Object.assign(Object.assign({}, state), { searchQuery: '', activeOptionIndex })
  },
  [ActionTypes.Search]: (state, action) => {
    if (state.disabled) return state
    if (state.listboxState === ListboxStates.Closed) return state
    let searchQuery = state.searchQuery + action.value.toLowerCase()
    let match = state.options.findIndex((option) => {
      var _a
      return (
        !option.dataRef.current.disabled &&
        ((_a = option.dataRef.current.textValue) === null || _a === void 0 ? void 0 : _a.startsWith(searchQuery))
      )
    })
    if (match === -1 || match === state.activeOptionIndex)
      return Object.assign(Object.assign({}, state), { searchQuery })
    return Object.assign(Object.assign({}, state), { searchQuery, activeOptionIndex: match })
  },
  ...
  [ActionTypes.UnregisterOption]: (state, action) => {
    ...
    let currentActiveOption = state.activeOptionIndex !== null ? nextOptions[state.activeOptionIndex] : null
...
export function Listbox(props) {
  let { value, onChange, disabled = false, horizontal = false } = props,
    passThroughProps = __rest(props, ['value', 'onChange', 'disabled', 'horizontal'])
  ...
  useIsoMorphicEffect(() => dispatch({ type: ActionTypes.SetOrientation, orientation }), [orientation])
  // Handle outside click
  useWindowEvent('mousedown', (event) => {
    var _a, _b, _c
    let target = event.target
    if (listboxState !== ListboxStates.Open) return
    if ((_a = buttonRef.current) === null || _a === void 0 ? void 0 : _a.contains(target)) return
    if ((_b = optionsRef.current) === null || _b === void 0 ? void 0 : _b.contains(target)) return
  ...
  let slot = useMemo(() => ({ open: listboxState === ListboxStates.Open, disabled }), [listboxState, disabled])
...
let Button = forwardRefWithAs(function Button(props, ref) {
  ...
  let handleClick = useCallback(
    (event) => {
      ...
      if (state.listboxState === ListboxStates.Open) {
        ...
        d.nextFrame(() => {
          var _a
          return (_a = state.buttonRef.current) === null || _a === void 0 ? void 0 : _a.focus({ preventScroll: true })
        })
      } else {
        event.preventDefault()
        dispatch({ type: ActionTypes.OpenListbox })
  ...
  let slot = useMemo(() => ({ open: state.listboxState === ListboxStates.Open, disabled: state.disabled }), [state])
...
function Label(props) {
  ...
  return render({
    props: Object.assign(Object.assign({}, props), propsWeControl),
    slot,
...
let Options = forwardRefWithAs(function Options(props, ref) {
  ...
  let labelledby = useComputed(() => {
    var _a, _b, _c
    return (_b = (_a = state.labelRef.current) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0
      ? _b
      : (_c = state.buttonRef.current) === null || _c === void 0
      ? void 0
      : _c.id
  }, [state.labelRef.current, state.buttonRef.current])
  let slot = useMemo(() => ({ open: state.listboxState === ListboxStates.Open }), [state])
  let propsWeControl = {
    'aria-activedescendant':
      state.activeOptionIndex === null
        ? undefined
        : (_a = state.options[state.activeOptionIndex]) === null || _a === void 0
        ? void 0
        : _a.id,
    'aria-labelledby': labelledby,
...
function Option(props) {
  let { disabled = false, value } = props,
    passthroughProps = __rest(props, ['disabled', 'value'])
  let [state, dispatch] = useListboxContext([Listbox.name, Option.name].join('.'))
  let id = `headlessui-listbox-option-${useId()}`
  let active = state.activeOptionIndex !== null ? state.options[state.activeOptionIndex].id === id : false
  ...
  useIsoMorphicEffect(() => {
    ...
    bag.current.textValue =
      (_b = (_a = document.getElementById(id)) === null || _a === void 0 ? void 0 : _a.textContent) === null ||
      _b === void 0
        ? void 0
        : _b.toLowerCase()
  }, [bag, id])
  ...
  useIsoMorphicEffect(() => {
    ...
    ;(_b = (_a = document.getElementById(id)) === null || _a === void 0 ? void 0 : _a.focus) === null || _b === void 0
      ? void 0
      : _b.call(_a)
  }, [state.listboxState])
  useIsoMorphicEffect(() => {
    ...
    d.nextFrame(() => {
      var _a, _b
      return (_b = (_a = document.getElementById(id)) === null || _a === void 0 ? void 0 : _a.scrollIntoView) ===
        null || _b === void 0
        ? void 0
  ...
  let slot = useMemo(() => ({ active, selected, disabled }), [active, selected, disabled])
  let propsWeControl = {
    id,
    role: 'option',
